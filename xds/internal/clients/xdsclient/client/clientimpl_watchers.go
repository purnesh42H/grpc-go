...
// A registry of xdsclient.ResourceType implementations indexed by their corresponding
// type URLs. Registration of an xdsclient.ResourceType happens the first time a watch
// for a resource of that type is invoked.
type resourceTypeRegistry struct {
	mu    sync.Mutex
	types map[string]xdsclient.ResourceType
}

func newResourceTypeRegistry(resourceTypes map[string]xdsclient.ResourceType) *resourceTypeRegistry {
	return &resourceTypeRegistry{types: resourceTypes}
}

func (r *resourceTypeRegistry) get(url string) xdsclient.ResourceType {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.types[url]
}

func (r *resourceTypeRegistry) maybeRegister(rType xdsclient.ResourceType) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (c *clientImpl) triggerResourceNotFoundForTesting(rType xdsclient.ResourceType, resourceName string) error {
	c.channelsMu.Lock()
...
func (c *clientImpl) resourceWatchStateForTesting(rType xdsclient.ResourceType, resourceName string) (ads.ResourceWatchState, error) {
	c.channelsMu.Lock()
	defer c.channelsMu.Unlock()
