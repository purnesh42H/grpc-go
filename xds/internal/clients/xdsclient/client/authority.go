...
type resourceState struct {
	watchers          map[xdsclient.ResourceWatcher]bool // Set of watchers for this resource.
	cache             xdsclient.ResourceData             // Most recent ACKed update for this resource.
...
type xdsChannelForADS func(*clients.ServerConfig, *authority) (*xdsChannel, func(), error)
...
type authority struct {
	...
	resources map[xdsclient.ResourceType]map[string]*resourceState
...
func newAuthority(args authorityBuildOptions) *authority {
	...
	ret := &authority{
		...
		resources:                 make(map[xdsclient.ResourceType]map[string]*resourceState),
		target:                    args.target,
	}
...
func (a *authority) adsStreamFailure(serverConfig *clients.ServerConfig, err error) {
...
func (a *authority) handleADSStreamFailure(serverConfig *clients.ServerConfig, err error) {
	if a.logger.V(2) {
...
// programming error).
func (a *authority) serverIndexForConfig(sc *clients.ServerConfig) int {
	for i, cfg := range a.xdsChannelConfigs {
		if clients.ServerConfigEqual(sc, cfg.serverConfig) {
			return i
		}
	}
...
// This method is called by the xDS client implementation (on all interested
// authorities) when a stream error is reported by an xdsChannel.
func (a *authority) adsResourceUpdate(serverConfig *clients.ServerConfig, rType xdsclient.ResourceType, updates map[string]ads.DataAndErrTuple, md xdsresource.UpdateMetadata, onDone func()) {
	a.xdsClientSerializer.TrySchedule(func(context.Context) {
...
//
// Only executed in the context of a serializer callback.
func (a *authority) handleADSResourceUpdate(serverConfig *clients.ServerConfig, rType xdsclient.ResourceType, updates map[string]ads.DataAndErrTuple, md xdsresource.UpdateMetadata, onDone func()) {
...
// does not exist.
func (a *authority) adsResourceDoesNotExist(rType xdsclient.ResourceType, resourceName string) {
	a.xdsClientSerializer.TrySchedule(func(context.Context) {
...
// not exist.
func (a *authority) handleADSResourceDoesNotExist(rType xdsclient.ResourceType, resourceName string) {
	if a.logger.V(2) {
...
func (a *authority) handleRevertingToPrimaryOnUpdate(serverConfig *clients.ServerConfig) {
	if a.activeXDSChannel != nil && clients.ServerConfigEqual(serverConfig, a.activeXDSChannel.serverConfig) {
...
func (a *authority) watchResource(rType xdsclient.ResourceType, resourceName string, watcher xdsclient.ResourceWatcher) func() {
	cleanup := func() {}
	...
	a.xdsClientSerializer.ScheduleOr(func(context.Context) {
		...
		if state == nil {
			...
			state = &resourceState{
				watchers:          make(map[xdsclient.ResourceWatcher]bool),
...
func (a *authority) unwatchResource(rType xdsclient.ResourceType, resourceName string, watcher xdsclient.ResourceWatcher) func() {
