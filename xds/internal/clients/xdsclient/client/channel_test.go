...
func xdsChannelForTest(t *testing.T, serverURI, nodeID string, watchExpiryTimeout time.Duration) *xdsChannel {
	...
	serverCfg := clients.ServerConfig{
		ServerURI:  serverURI,
		Extensions: grpctransport.ServerConfigExtension{Credentials: insecure.NewBundle()},
	}
	tr, err := (&grpctransport.Builder{}).Build(serverCfg)
	if err != nil {
		t.Fatalf("Failed to create a transport for server config %v: %v", serverCfg, err)
	}

	xdsClientConfig := xdsclient.Config{
		Servers: []clients.ServerConfig{{ServerURI: serverURI}},
...
func TestChannel_ADS_HandleResponseFromManagementServer(t *testing.T) {
	const (
