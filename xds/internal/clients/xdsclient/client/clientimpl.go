...
type clientImpl struct {
	...
	config             *xdsclient.Config            // Complete xDS client configuration.
...
func newClientImpl(config *xdsclient.Config, watchExpiryTimeout time.Duration, streamBackoff func(int) time.Duration, target string) (*clientImpl, error) {
...
func (c *clientImpl) getChannelForADS(serverConfig *clients.ServerConfig, callingAuthority *authority) (*xdsChannel, func(), error) {
	if c.done.HasFired() {
...
//
// A non-nil error is returned if an xdsChannel was not created.
func (c *clientImpl) getChannelForLRS(serverConfig *clients.ServerConfig) (*xdsChannel, func(), error) {
	if c.done.HasFired() {
		return nil, nil, ErrClientClosed
	}
...
// created.
func (c *clientImpl) getOrCreateChannel(serverConfig *clients.ServerConfig, initLocked, deInitLocked func(*channelState)) (*xdsChannel, func(), error) {
	c.channelsMu.Lock()
	...
	if state, ok := c.xdsActiveChannels[clients.ServerConfigString(serverConfig)]; ok {
		if c.logger.V(2) {
	...
	if err != nil {
		return nil, func() {}, fmt.Errorf("xds: failed to create transport for server config %s: %v", clients.ServerConfigString(serverConfig), err)
	}
	state := &channelState{
		parent:                c,
	...
	if err != nil {
		return nil, func() {}, fmt.Errorf("xds: failed to create xdsChannel for server config %s: %v", clients.ServerConfigString(serverConfig), err)
	}
	state.channel = channel
	c.xdsActiveChannels[clients.ServerConfigString(serverConfig)] = state
	initLocked(state)
	return state.channel, c.releaseChannel(serverConfig, state, deInitLocked), nil
}
...
func (c *clientImpl) releaseChannel(serverConfig *clients.ServerConfig, state *channelState, deInitLocked func(*channelState)) func() {
	return sync.OnceFunc(func() {
		...
		delete(c.xdsActiveChannels, clients.ServerConfigString(serverConfig))
		if c.logger.V(2) {
...
func (c *clientImpl) dumpResources() *v3statuspb.ClientConfig {
	...
	return &v3statuspb.ClientConfig{
		Node:              clients.NodeProto(c.config.Node),
...
func (cs *channelState) adsResourceUpdate(typ xdsclient.ResourceType, updates map[string]ads.DataAndErrTuple, md xdsresource.UpdateMetadata, onDone func()) {
	if cs.parent.done.HasFired() {
		return
	}
...
func (cs *channelState) adsResourceDoesNotExist(typ xdsclient.ResourceType, resourceName string) {
	if cs.parent.done.HasFired() {
		return
	}
